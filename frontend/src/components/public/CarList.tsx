    ...
    <div>
      ...
      {hasMore && !isLoading && (
        <div className="mt-6 sm:mt-8 text-center">
          <button
            onClick={loadMore}
            className="inline-flex items-center gap-2 rounded-full bg-[#F4B400] px-6 sm:px-8 py-2.5 sm:py-3 text-sm font-semibold text-black transition-all hover:scale-105 hover:shadow-xl active:scale-95 disabled:opacity-50"
          >
            Load More
      ...
      {!hasMore && cars.length > 0 && (
        <p className="mt-6 sm:mt-8 text-center text-sm text-body">
...
const SkeletonGrid = ({ count = 6 }: { count?: number }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
    {Array.from({ length: count }).map((_, i) => (
      <div key={i} className="rounded-2xl sm:rounded-3xl border border-line bg-white p-4 animate-pulse">
