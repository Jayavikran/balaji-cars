    ...
    <div className="min-h-screen flex flex-col">
      ...
      {featured.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-4">
          ...
          {/* Featured cars - 2 columns on mobile, 4 on desktop */}
      ...
      <section id="car-listings" className="scroll-mt-24 sm:scroll-mt-20 max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-8 flex-1">
        <div className="flex items-center justify-between mb-4 sm:mb-5 flex-wrap gap-2 sm:gap-3">
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <h2 className="font-display text-lg sm:text-xl font-bold text-ink dark:text-white">{resultsHeading()}</h2>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {data && <span className="text-xs sm:text-sm text-body">{data.pagination.total} vehicles</span>}
        ...
        {isLoading ? (
          ...
          <>
            {/* Mobile: 2 columns with 12px gap, Desktop: 4 columns with 20px gap */}
            <div className={`grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 transition-opacity duration-200 ${showBackgroundSpinner ? 'opacity-60' : 'opacity-100'}`}>
          ...
          <div className="text-center py-12 sm:py-16 border border-dashed border-line dark:border-white/10 rounded-2xl">
            <p className="text-sm sm:text-base text-body">No cars matched your search. Try adjusting your filters.</p>
          </div>
        )}
      </section>
